...
export function SubmissionStatusForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setState("error");
      setMessage(payload.error ?? "We could not retrieve your status. Please try again.");
      return;
    }
    setResult(payload);
    setState("success");
  ...
  return (
    <form className="submission-form status-form" onSubmit={submit}>
      ...
      <div className="form-submit"><p>For privacy, we show a status only when both the reference code and original submission email match.</p><button className="button button-dark" disabled={state === "checking"}>{state === "checking" ? "Checking…" : "Check status"}</button></div>
    </form>
  );
}
