...
export function SubmissionForm() {
  ...
  if (status === "success") {
    return <section className="submission-success" aria-live="polite"><p className="handwritten">submission received</p><h2>We have your work.</h2><p>{message}</p><p>An editor will be in touch after the first read, usually within seven days.</p><div className="success-actions"><Link className="button button-dark" href="/status">Check submission status</Link><button className="button button-paper" onClick={() => { setStatus("idle"); setMessage(""); }}>Start another submission</button></div></section>;
  }
    ...
    <form className="submission-form" onSubmit={submit} encType="multipart/form-data">
      ...
      <fieldset>
        ...
        <div className="form-grid">
          ...
          <label className="wide"><input name="guardianConfirmed" type="checkbox" onChange={(event) => setRequiresGuardian(event.target.checked)} /> The author is under 18 and has a parent or guardian&apos;s approval. For authors under 13, a parent or guardian must complete this form using their own email.</label>
      ...
      <div className="form-submit"><p>Free to submit. Editors read every submission before review.</p><button className="button button-dark" disabled={status === "sending"}>{status === "sending" ? "Sending…" : "Submit work"}</button></div>
    </form>
  );
}
