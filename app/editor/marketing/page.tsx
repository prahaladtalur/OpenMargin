...
import { ensureOperationsTables, ensureSubmissionTable, getDb } from "../../../db";
import { partnerInquiries, reviewerApplications, submissions } from "../../../db/schema";
import { editorSignOutPath, requireEditor } from "../../../lib/editor-auth";
import { CopyCampaignLinkButton } from "./CopyCampaignLinkButton";
...
export default async function MarketingDeskPage() {
  ...
  const [rows, reviewerRows, partnerRows] = await Promise.all([
    getDb().select({
    ...
    getDb().select({ status: reviewerApplications.status }).from(reviewerApplications).limit(500),
    getDb().select({ status: partnerInquiries.status }).from(partnerInquiries).limit(500),
  ]);
  ...
  return (
    <main className="editor-page">
      ...
      <section className="editor-list marketing-links"><div className="editor-list-heading"><p className="eyebrow">Ready-to-share links</p><p>Use one link per audience. Keep the destination visible.</p></div><div className="marketing-link-grid">{campaignLinks.map((link) => { const url = `https://openmargin.org${link.path}`; return <article key={link.path}><p className="editor-reference">{link.audience}</p><h2>{link.label}</h2><code>{url}</code><div className="marketing-link-actions"><a className="button button-paper" href={link.path}>Open link</a><CopyCampaignLinkButton url={url} /></div></article>; })}</div></section>
