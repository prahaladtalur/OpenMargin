      ...
      <section className="editor-list">
        ...
        {rows.length === 0 ? <div className="editor-empty"><h2>No submissions yet.</h2><p>New submissions will appear here as soon as the portal receives them.</p></div> : rows.map((row) => (
          <article className="editor-card" key={row.id}>
            ...
            <div className="editor-data-grid"><div><h3>Author</h3><p>{row.authorName}<br /><a href={`mailto:${row.authorEmail}`}>{row.authorEmail}</a><br />Age {row.ageBand}</p></div><div><h3>Guardian</h3><p>{row.guardianEmail ? <a href={`mailto:${row.guardianEmail}`}>{row.guardianEmail}</a> : "Not required"}</p></div><div><h3>School / region</h3><p>{row.schoolOrOrganization ?? "Not supplied"}<br />{row.countryOrRegion ?? "Not supplied"}</p></div><div><h3>Origin</h3><p>{row.originNote}</p></div></div>
